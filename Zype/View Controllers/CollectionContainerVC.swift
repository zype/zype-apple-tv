...
class ShowCollectionItem: CollectionLabeledItem {
    init(value: PlaylistModel) {
        ...
        if(value.images.count > 0) {
            self.imageURL = NSURL(string: value.images.first!.imageURL)
        } else if (value.thumbnails.count > 0) {
            self.imageURL = NSURL(string: value.thumbnails.first!.imageURL)
        } else {
            self.imageURL = NSURL(string: "http://placehold.it/320x180")
        }
        self.object = value
    }
}
